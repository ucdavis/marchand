...
elseif($c) {
	...
	if(!$major_title_url && !$minor_title_url) {
		...
		while ($myrow_maj = mysql_fetch_row($result_maj)) {
			...
			$result = mysql_query("SELECT * FROM Images WHERE ImageID = '$myrow_maj[1]' AND Collection = '$collection_id' AND Current = '1'",$db);
			while ($myrow = mysql_fetch_row($result)) {
				## Get Major Title
				...
				print "<LI><A HREF='/ic/collection/$c/$major_title_url/'>$major_title</A>";
				}
			}
		}
	...
	elseif($major_title_url && !$minor_title_url) {
		...
		while ($myrow_minor_id = mysql_fetch_row($result_minor_id)) {
			...
			$major_title_url=str_replace(" ","_", $major_title);
			print "<LI><A HREF=/ic/collection/$c/$major_title_url/$minor_title_url/>$minor_title</A>";
			}
		}

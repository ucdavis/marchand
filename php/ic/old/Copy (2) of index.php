...
elseif($s) {
	...
	## Get standard ID from standard
		...
		while ($myrow = mysql_fetch_row($result)) {
			...
			$result_details = mysql_query("SELECT * FROM Images WHERE ImageID = '$myrow[2]' AND Current = '1' LIMIT 1",$db);
...
elseif($c) {
	...
	if(!$major_title_url && !$minor_title_url) {
		...
		## List the Majors
		$result_maj = mysql_query("SELECT * FROM Majors_Minors GROUP BY Major",$db);
		while ($myrow_maj = mysql_fetch_row($result_maj)) {

			## Get a sample image from this and see which collection it is in
			$result = mysql_query("SELECT * FROM Images WHERE ImageID = '$myrow_maj[1]' AND Collection = '$collection_id'",$db);
			while ($myrow = mysql_fetch_row($result)) {
				## Get Major Title
				...
				if($uid == 'kev' || $uid == 'safutrel') { print " - $major_code"; }
				print "</A>";
				}
			}
		}
	...
	elseif($major_title_url && !$minor_title_url) {
		...
		while ($myrow_minor_id = mysql_fetch_row($result_minor_id)) {
			...
			if($uid == 'kev' || $uid == 'safutrel') { print " - $minor_code"; }
			print "</A>";
			}
		}
	...
	elseif($major_title_url && $minor_title_url && !$sid) {
		...
		while ($myrow_major_title = mysql_fetch_row($result_major_title)) {
			...
			$result = mysql_query("SELECT * FROM Images WHERE ImageID = '$myrow_major_title[0]'  AND Current = '1' LIMIT 1",$db);
			while ($myrow = mysql_fetch_row($result)) {
	...
	while ($myrow = mysql_fetch_row($result)) {
		...
		if($myrow[2] != 'IMG0000.jpg') {
			print "<TABLE BORDER=0>
				<TR><TD COLSPAN=2><A HREF=/marchandslides.bak/$myrow[1]/images/$myrow[2]><IMG BORDER=0 WIDTH=900 SRC=/marchandslides.bak/$myrow[1]/images/$myrow[2]></A></TD></TR>
...
function mainmenu($db) {
	...
	## Search box
		...
		print "</SELECT><INPUT TYPE=SUBMIT></FORM><HR>";
		...
		while ($myrow = mysql_fetch_row($result)) {
			$count=0; 
			## Get image count
			$result_count = mysql_query("SELECT COUNT(*) FROM Images WHERE Collection = '$myrow[0]' AND Current = '1'",$db);
...
function thumbnail($url,$img,$text,$cit) {
	global $uid,$sid,$redirect,$minor_code,$major_code,$old_num;

	#print "<DIV ID=ic_thumb>";
	if($uid == 'kev' || $uid == 'safutrel') { print "<TR><TD><A NAME=$sid></A></TD></TR>"; }
	print "<TR VALIGN=TOP BGCOLOR=EEEEEE><TD><CENTER><DIV CLASS=wrap1><DIV CLASS=wrap2><DIV CLASS=wrap3>";
	print "<A HREF=$url><IMG BORDER=0 SRC=$img></A>";
	print "</DIV></DIV></DIV></TD>";
	if($uid == 'kev' || $uid == 'safutrel') { print "<TD><A HREF=/secure/admin/edit_slides.php?sid=$sid&W=edit&redirect=$redirect>edit<BR>$major_code-$minor_code-$old_num</A></TD>"; }
	print "<TD>$text<P><I>$cit</I></TD></TR>";
	}
...
function standard_count($db,$what) {
	...
	while ($myrow = mysql_fetch_row($result)) {
		...
		while ($myrow_sub = mysql_fetch_row($result_sub)) {
			...
			$q="SELECT COUNT(*) FROM Standards_Data, Images WHERE Images.ImageId = Standards_Data.ImgID AND Current = '1' AND SID = '$myrow_sub[0]'";

<?php
	...
	require_once "/home/hcpweb/historyproject.ucdavis.edu/classes/Connection.php";
	require_once '../app/ic.inc.php';
	$Connection = new Connection();
	
	$quote = $Connection->GetRandomQuote();
	
	include "/home/hcpweb/historyproject.ucdavis.edu/snippets/header.htm"; 
	include "/home/hcpweb/historyproject.ucdavis.edu/snippets/navigation.php";
?>
...
<?php
	...
	if($uid == 'ptindall' || $uid == 'safutrel' || $uid == 'cthielen' || $uid == 'tacrowde' || $uid == 'lkraus') {
		$editor = 1;
		$edit = "or public = 0";
	} else {
		$editor = 0;
			...
			while ($myrow = mysql_fetch_assoc($result)) {
				...
				while ($myrow_details = mysql_fetch_assoc($result_details)) {
					...
					if($myrow_details['thumbnail'] != 'IMG0000.jpg') { 
						...
						thumbnail($url, $img, $myrow_details['title'], $myrow_details['card']."<br /><hr style=\"border-color: #999;\" /><br />".$myrow['citation'], $major_code, $minor_code); 
					}
					$count++;
				}
			}
		...
		elseif($major_title_url && $minor_title_url && !$sid) {
			...
			while ($myrow_major_title = mysql_fetch_row($result_major_title)) {
				...
				while($myrow = mysql_fetch_assoc($result)) {
					...
					thumbnail($url, $img, $myrow['title'], $myrow['card']."<br /><hr style=\"border-color: #999;\" /><br />".$myrow['citation'], $major_code, $minor_code); 

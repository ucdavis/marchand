...
function redirect_to_search(remember_page, specified_index) {
	...
	if(($("input[name=search_term_id]").val() != undefined) && ($("input[name=search_term_id]").val() > 0)) {
		...
		window.location.href = build_search_url(specified_index);
	}
}
...
function build_search_url(specified_index, specified_per_page) {
	...
	return 'http://historyproject.ucdavis.edu/ic/index.php?region=' + region + '&topic=' + topic + '&standard_cal=' + standard_cal + '&collection=' + collection + "&index=" + specified_index + "&per_page=" + specified_per_page + "&query=" + escape(query);
}
